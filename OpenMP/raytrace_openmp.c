...
int main() {
    printf("Starting OpenMP Ray Tracer...\n");
    ...
    // MAIN PARALLELIZATION: Parallelize the outer loop (rows)
    #pragma omp parallel for schedule(dynamic, 1)
    for (int y = 0; y < IMAGE_HEIGHT; y++) {
        // Progress reporting - only from thread 0 to avoid race conditions
        if (omp_get_thread_num() == 0 && (y + 1) % 50 == 0) {
    ...
    free(image);
    printf("OpenMP Ray tracing completed!\n");
