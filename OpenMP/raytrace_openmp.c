...
int main(int argc, char* argv[]) {
    ...
    printf("Starting Ray Tracer (OpenMP Parallel Version)...\n");
    ...
    // Start timing (using omp_get_wtime for better precision)
    double start_time = omp_get_wtime();
    
    // ===== PARALLELIZATION USING OPENMP =====
    // The outer loop (rows) is parallelized to distribute work across threads
    // Each thread processes different rows independently
    // The schedule(dynamic) clause ensures good load balancing
    ...
    for (int y = 0; y < IMAGE_HEIGHT; y++) {
        ...
        // Progress indicator (protected by critical section to avoid race conditions)
        #pragma omp critical
        {
            if ((y + 1) % 50 == 0) {
    ...
    double rendering_time = end_time - start_time;
    printf("Rendering time: %.4f seconds\n", rendering_time);
    
    // Save image
    write_ppm("raytrace_openmp.ppm", image);
